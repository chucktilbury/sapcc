...
### Rule structure
The structure of rules is simple. The first item encountered must be the unique name of a non-terminal symbol. Following that, a code block is required. This may be empty, but code that is in the block will be appended to the function that builds the syntax tree. Next, a colon ':' tells the system that a rule clause is next. There can be one or more clauses. The clause is a list of terminal and non-terminal symbols that specify the structure of the rule. Then another code block is present that may be blank. This code block is appended to the end of the function that traverses the syntax tree. 
